...
const MasteryActivityItem = ({
  ...
  return (
    ...
    >
      <CardTop onClick={onToggle}>
        <div>
          <ActivityTitle>{activity.name}</ActivityTitle>
          <ActivityMeta>
            <span>Rank: {activity.current_level}</span>
          </ActivityMeta>
        </div>
      ...
      {isExpanded && (
        <>
            ...
            <StatsPanel>
              <StatsGrid>
                <StatBox>
                  ...
                  <StatValue>
                    {prettyHours(activity.stats.today_hours)}h
                  </StatValue>
                </StatBox>
                ...
                <StatBox>
                  ...
                  <StatValue>
                    {prettyHours(activity.stats.weekly_hours)}h
                  </StatValue>
                </StatBox>
              ...
              {activity.next_milestone ? (
                <NextMilestoneCard>
                  <div>
                    <MilestoneBadge>
                      {activity.next_milestone.label}
                    </MilestoneBadge>
                    <MilestoneMeta>
                      {prettyHours(activity.next_milestone.distance_hours)}h
          ...
          <AddCustomHoursForm>
            ...
            <Button
              variant="secondary"
              onClick={() => onAddHours(activity.id, 1)}
            >
              +1h
            </Button>
            <Button
              variant="secondary"
              onClick={() => onAddHours(activity.id, 2)}
            >
              +2h
            </Button>
            <Button
              variant="secondary"
              onClick={() => onAddHours(activity.id, 5)}
            >
              +5h
            </Button>

            <Button onClick={() => onSubmitQuickAdd(activity.id)}>
              Log time
            </Button>
          </AddCustomHoursForm>
        </>
