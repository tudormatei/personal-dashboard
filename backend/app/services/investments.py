...
async def request_financial_data(start_date, end_date):
    ...
    if not token or not query_id:
        raise ValueError("Set FLEX_TOKEN and QUERY_ID environment variables first")
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        for _ in range(10):
            async with session.get(
                f"{BASE_URL}/GetStatement", params=get_statement_params
            ) as resp:
...
def build_final_report(all_reports: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    response["account"]["currency"] = all_reports[0]["FlexQueryResponse"][
        "FlexStatements"
    ]["FlexStatement"]["AccountInformation"]["@currency"]
    response["account"]["type"] = all_reports[0]["FlexQueryResponse"]["FlexStatements"][
        "FlexStatement"
    ]["AccountInformation"]["@accountType"]
    response["account"]["id"] = all_reports[0]["FlexQueryResponse"]["FlexStatements"][
        "FlexStatement"
    ]["AccountInformation"]["@accountId"]

    # one time open positions from last date
    open_positions = all_reports[-1]["FlexQueryResponse"]["FlexStatements"][
        "FlexStatement"
    ...
    response["cashReport"]["starting"] = all_reports[0]["FlexQueryResponse"][
        "FlexStatements"
    ]["FlexStatement"]["CashReport"]["CashReportCurrency"]["@startingCash"]
    response["cashReport"]["ending"] = all_reports[-1]["FlexQueryResponse"][
        "FlexStatements"
    ]["FlexStatement"]["CashReport"]["CashReportCurrency"]["@endingCash"]
    ...
    for report in all_reports:
        ...
        statement_equity = statement["EquitySummaryInBase"][
    ...
    response["timeWeightedReturn"] = compute_time_weighted_return(
        response["valueOverTime"], response["statementFunds"]
    )
...
def compute_time_weighted_return(value_over_time, statement_funds):
    ...
    twr_timeseries = [{"date": value_over_time[0]["date"], "twr": 0.0}]
    ...
    for i in range(1, len(value_over_time)):
        ...
        twr_timeseries.append({"date": date, "twr": round((twr - 1) * 100, 2)})

        prev_value = curr_value

    total_twr = (twr - 1) * 100
    return {"total": round(total_twr, 2), "series": twr_timeseries}


def run_monte_carlo_simulation(
    start_portolio_value,
    twr_series,
    monthly_deposit,
    monthly_withdrawal,
    days_ahead,
    ...
    projections = {p: np.percentile(simulated_paths, p, axis=1) for p in percentiles}
    ...
    response = {
        ...
        "goalAchievement": (
            {"targetValue": target_value, "successProbability": success_probability}
            if target_value
            else None
        ),
    }
