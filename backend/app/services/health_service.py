...
from ..constants.constants import AVERAGE_DAYS_PER_MONTH, AVERAGE_LIFTING_CALORIES, LIFTING_SESSIONS_PER_WEEK, MACRO_TYPES, RELEVANT_RECORD_TYPES, RELEVANT_WORKOUT_TYPES
from ..repositories.health_repository import fetch_records, fetch_workouts, insert_records, insert_workouts
...
async def process_health_file(contents: bytes) -> int:
    ...
    return len(records) + len(workouts)


def parse_apple_health_xml(file_like) -> Tuple[List[HealthRecord], List[WorkoutSession]]:
...
def get_weight_stats(
    ...
    df_daily = pd.DataFrame({
        'date': grouped['date'].min().values,
        'value': grouped['value'].mean().values
    }).sort_values('date')
    ...
    return json.loads(df_daily.to_json(
...
def get_macros_stats(
    ...
    end_date: Optional[str] = None,
    ma_windows: Optional[str] = "7,30,90",
    target_values: Optional[str] = None
) -> Dict[str, List[Dict]]:
    ...
    target_dict = {}
    if target_values:
        for pair in target_values.split(","):
            k, v = pair.split(":")
    ...
    for nutrient_name, record_type in MACRO_TYPES.items():
        ...
        result[nutrient_name] = compute_nutrient_stats(
            ...
            target_value=target_dict.get(
                nutrient_name) if target_dict else None
        )
...
def compute_nutrient_stats(
    ...
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    ma_windows: Tuple[int, ...] = (7, 30, 90),
    nutrient_name: str = "value",
    ...
    for window in ma_windows:
        df_daily[f'ma{window}'] = df_daily['value'].rolling(
            window).mean()

    if target_value is not None:
        df_daily[f'to_target'] = target_value - \
            df_daily['value']
    ...
    df_daily[numeric_cols] = df_daily[numeric_cols].round(0)

    return {
        "daily": json.loads(df_daily.to_json(orient='records', date_format='iso')),
...
def estimate_maintenance(
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    min_daily_calories: float = 2500,
    calories_ma_window: Optional[int] = 3
) -> dict:
    ...
    if not weight_records or not calorie_records:
        return {"estimated_maintenance_calories": None, "message": "Insufficient data"}
    ...
    if len(df) < 2:
        return {
            "estimated_maintenance_calories": None,
            "message": f"Not enough valid data: only {len(df)} days meet criteria"
        }

    if calories_ma_window:
        df['calories_ma'] = df['calories'].rolling(
            window=calories_ma_window, min_periods=1, center=True).mean()
    else:
    ...
    daily_exercise_calories = (
        LIFTING_SESSIONS_PER_WEEK * AVERAGE_LIFTING_CALORIES) / 7
    maintenance_calories = avg_daily_calories + \
        daily_exercise_calories - daily_adjustment
...
def get_workout_stats(
    ...
    df_daily = df.groupby(df['start'].dt.date)[
        'duration_min'].sum().reset_index()
    df_daily.rename(
        columns={'start': 'date', 'duration_min': 'duration'}, inplace=True)
    df_daily['duration'] = df_daily['duration'].round(2)
    df_daily['date'] = pd.to_datetime(
        df_daily['date']).dt.strftime('%Y-%m-%dT%H:%M:%S')
    ...
    sessions_per_weekday = df['weekday'].value_counts().reindex(
        weekday_order).to_dict()
    ...
    sessions_per_month = {str(k): int(v)
                          for k, v in sessions_per_month.items()}

    full_dates = pd.date_range(
        start=df_daily['date'].min(), end=df_daily['date'].max())
    ...
    return {
        "daily_stats": json.loads(df_daily.to_json(orient='records', date_format='iso')),
